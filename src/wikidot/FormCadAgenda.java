...
public class FormCadAgenda extends JDialog {
	...
	private boolean edicao = false; /*
									 * variável de teste que será entendita mais
									 * à frente
									 */
	...
	void inicia() { // este método é responsável por mostrar o formulário na
		...
		// define posição do formulário(x,y) e altura e largura de cada
		// component.
	...
	private void buttonGravarActionPerformed(ActionEvent e) {
		int resposta;
		/*
		 * verifica se já é um arquivo novo pra inclusão ou apenas edição e um
		 * já cadastrado
		 */
		if (edicao == false) {
			...
			if (resposta == JOptionPane.OK_OPTION) {
				...
				// adiciona uma linha nova de cadastro na tela pricipal da
				// agenda
				pai.addRow(new String[] { p.getCodigo(), p.getNome(), p.getTelefone(), p.getEmail(), p.getEndereco() });

				this.dispose();
			}
		} else { // se edicao = true indica que é uma alteração.
			resposta = JOptionPane.showConfirmDialog(this, "Salvar Alterações", "Edição", JOptionPane.OK_CANCEL_OPTION,
					JOptionPane.QUESTION_MESSAGE);

			if (resposta == JOptionPane.OK_OPTION) {
				Pessoa p = Main.minhaAgenda.getPessoa(codigo);

				p.setValores(new String[] { textNome.getText(), textTelefone.getText(), textEmail.getText(),
						textEndereco.getText() });

				JOptionPane.showMessageDialog(this, "Edição efetuada!!!", "Edição", JOptionPane.INFORMATION_MESSAGE);

				pai.atualizaTabela(
						new String[] { p.getCodigo(), p.getNome(), p.getTelefone(), p.getEmail(), p.getEndereco() });
